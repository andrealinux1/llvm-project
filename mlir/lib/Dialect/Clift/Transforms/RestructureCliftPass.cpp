...
#include "llvm/ADT/FilteredGraphTraits.h"
...
class RestructureCliftRewriter : public OpRewritePattern<LLVM::LLVMFuncOp> {
  ...
  bool outlineOnRegionTree(RegionTree &Rt,
    ...
    for (RegionNode *ParentRegion : post_order(&*(Rt.rbegin()))) {
      ...
      for (RegionNode *ChildRegion : ParentRegion->successor_range()) {
        ...
        // 2) The entry node is a `ChildRegionDescriptor`. In this situation, we
        //    can deduce the following: all outlining opportunities that are
        //    situated in the child region, should have already taken care by
        //    the outlining for that region. It is as the boundaries of the
        //    child region are opaque for us here.
        //    We know however that the block containing the `clift.loop` which
        //    will be generated by this region, will be the entry of the current
        //    region.
        //    We should therefore proceed with the outlining, but considering
        //    that the entry node is not already present, but will be
        //    eventually, so every other late entry should be outlined.
        //    It suffices to pass to the remaining part of the code `nullptr`,
        //    so that every entry candidate found will be outlined.
        ...
        llvm::SmallVector<std::pair<mlir::Block *, mlir::Block *>>
            LateEntryPairs = ChildRegion->getOutlinedEntries(
                EntryCandidates, Entry, ParentRegion);
        ...
        for (mlir::Block *OutlinedBlock : OutlinedNodes) {
          ParentRegion->insertElement(OutlinedBlock);
        }
      }
    }
  ...
  void generateCliftGotoSuccessors(
    ...
    // inside the `clift.loop`, is also in the same region.
    for (mlir::Block &CliftLoopBlock : LoopRegion) {
